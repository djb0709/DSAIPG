...
public class BinaryHeapBenchMark {
    ...
    public static void main(String[] args) throws PQException {
        PriorityQueue<Integer> pq = new PriorityQueue<>(M, true, Comparator.comparing(Integer::intValue), false);

        Supplier<PriorityQueue<Integer>> supplier = () -> {
            return new PriorityQueue<>(M, true, Comparator.comparing(Integer::intValue), false);
        };

        Benchmark_Timer<PriorityQueue<Integer>> benchmark = new Benchmark_Timer<>(
                "PQ Basic Binary Heap",
                pqInstance -> {
                    // -- insert --
        ...
        //
        System.out.printf("BinaryHeap (M=%d, inserts=%d, removes=%d), average total time: %.5f ms%n",
